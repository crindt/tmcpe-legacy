...
class TestbedRoleController {
	...
	def list = {
		...
		[authorityList: TestbedRole.list(params)]
	}
	 ...
	 */
	def show = {
		def authority = TestbedRole.get(params.id)
		if (!authority) {
			flash.message = "TestbedRole not found with id $params.id"
	...
	def delete = {
		...
		flash.message = "TestbedRole $params.id deleted."
		redirect action: list
	}
	...
	def update = {
		...
		if (authority.version > version) {
			authority.errors.rejectValue 'version', 'authority.optimistic.locking.failure',
				'Another user has updated this TestbedRole while you were editing.'
