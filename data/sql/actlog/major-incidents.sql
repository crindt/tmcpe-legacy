...
CREATE VIEW linked_sigalerts AS
       SELECT sla.*,tvd.id,tvd.name,process_xs(xs) as xs,str_compare_trim( name, (process_xs(xs))[1] ) AS sct 
...
CREATE VIEW matched_sigalerts AS
       SELECT cad, id,q.name,xs
       FROM ( SELECT * FROM matched_sigalerts ) q	
       	      WHERE sct=1 GROUP BY cad;
