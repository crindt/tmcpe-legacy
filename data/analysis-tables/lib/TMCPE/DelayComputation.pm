...
#     scalar => [ { -default => "localhost" }, 'gams_host' ],
     ...
     scalar => [ qw/ cad facil dir pm vdsid / ],
     ...
     scalar => [ qw/ tot_delay avg_delay net_delay / ],
...
sub get_pems_data {
    ...
    my $calcend   = Mktime(Add_Delta_DHMS(@tt[0..5],0,0,$self->prewindow,0));
...
#    my $resf = io( "/usr/local/src/lp_solve_5.5/lp_solve/lp_solve -presolve -wmps $mpsname < $self->fname|" );
    ...
    print "SOLVING...";
    system( "ssh $gu\@$gh 'cd tmcpe/work && /cygdrive/c/Progra~1/GAMS22.2/gams.exe $gf'" );
    print "done\n";
...
sub write_to_db {
    ...
    my $ia;
    eval {
	$ia = $self->tmcpe_db->resultset( 'IncidentFacilityImpactAnalysis' )->create(
	    {
		version => 1,
	...
	my $as;
	eval { $as = $ia->create_related( 'analyzed_sections', { section_id=> $station->{vds}->id, version=>1 } ); };
	croak $@->{msg} if $@;

	foreach my $secdat ( sort { $a->{timeofday} cmp $b->{timeofday} } @{$station->{data}} ) {
	    my $at;
	    eval { 
		$at = $as->create_related( 'analyzed_timestep', 
					   {
					       version => 1,
					       fivemin => $secdat->{fivemin},
					       vol => $secdat->{incflw},
					       spd => $secdat->{incspd},
					       occ => -0,
