...
psql -U postgres -t  tmcpe_test -c "select section_id,mintime,maxtime from (select cad,section_id, mintime,maxtime,case when good = 0 THEN 10000000000000 ELSE 1.0*bad/good END as ratio from ( select * from find_data_gaps2 where mintime > '2009-03-01') q order by mintime asc,cad) qq where ratio>.95" | perl -F'/\s*\|\s*/'  -ane 'my @args = map { s/^\s*//g;chomp($_);$_ } @F; system "ssh crindt\@***REMOVED*** cd pems \\\; perl push-pems-5min.pl --vds=$args[0] --from=\\\"$args[1]\\\" --to=\\\"$args[2]\\\"\n" if ( @args && $args[1] ne "" && $args[2] ne "" );' 
